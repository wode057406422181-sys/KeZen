...
use super::kezen_proto::{
    kezen_agent_client::KezenAgentClient, ClientMessage, ServerMessage,
};
pub async fn run_grpc_client(
    ...
    // 1. Connect to server
    let mut client = KezenAgentClient::connect(url.clone()).await
    ...
    let response = client.stream_session(tonic::Request::new(request_stream)).await?;
    ...
    match action {
        ...
        UserAction::PermissionResponse { id, allowed, always_allow } => {
            let decision = if allowed {
                if always_allow {
                    Some(super::kezen_proto::permission_response::Decision::AlwaysAllow(
                        super::kezen_proto::AlwaysAllow { suggestion_index: 0 } // Re-check this if you use suggestion_index later
                    ))
                } else {
                    Some(super::kezen_proto::permission_response::Decision::AllowOnce(
            ...
            Some(ClientMessage {
                action: Some(super::kezen_proto::client_message::Action::PermissionResponse(
...
fn server_message_to_engine_event(msg: ServerMessage) -> Option<EngineEvent> {
    ...
    match msg.event {
        ...
        Some(Event::CompactProgress(c)) => Some(EngineEvent::CompactProgress { message: c.message }),
